...
router.put('/:userId/upgrade-milestone', async (req, res) => {
    try {
        ...
        if (creature1.upgrade_progress > 0 && creature2.upgrade_progress > 0 && 
            ...
            if (creatureTemplate.type === 'common') {
                waitTimeMinutes = targetLevel === 11 ? 2 : targetLevel === 21 ? 5 : 10; // Changed from 15/30/60 to 2/5/10
            } else if (creatureTemplate.type === 'rare') {
            ...
            if (timeSinceLastClick < requiredWaitTimeSeconds) {
                ...
                return res.status(200).json({
                    success: true,
                    message: `Timer started. Wait ${waitTimeMinutes} minutes before collecting. No progress yet.`,
        ...
        // Set partner IDs but IMPORTANT: don't set progress on first call
        if (!isSecondClick) {
            // First evolve call: only set partner IDs and timer, no progress
            ...
            // Create active_merges entry with 0% progress if it doesn't exist
            if (!user.active_merges) {
                user.active_merges = [];
            }
        ...
        if (creatureTemplate.type === 'common') {
            ...
            // Add or update the active merge record
            ...
            if (existingMergeIndex !== -1) {
                ...
                user.active_merges[existingMergeIndex].progress = 0; // Reset progress
        ...
        // *** MAJOR FIX: Check for any creature with existing progress BEFORE creature-specific checks ***
        // Define isPartnered by checking if creatures are already partnered
        const isPartnered = 
            (creature1.upgrade_partner_id && creature1.upgrade_partner_id.toString() === creature2._id.toString()) ||
            (creature2.upgrade_partner_id && creature2.upgrade_partner_id.toString() === creature1._id.toString());
        
        // If there's existing progress after a collect, we should ALWAYS allow immediate evolve
        if (isPartnered && previousProgress > 0 && previousProgress < 100) {
            console.log(`Continuing evolution with ${previousProgress}% progress for ${creatureTemplate.type} creature`);
            
            // Get wait time based on creature type and level
            let waitTimeMinutes;
            if (creatureTemplate.type === 'common') {
                waitTimeMinutes = targetLevel === 11 ? 2 : targetLevel === 21 ? 5 : 10;
            } else if (creatureTemplate.type === 'rare') {
            ...
            if (user.active_merges) {
                ...
                if (activeMergeIndex !== -1) {
                    ...
                    user.active_merges[activeMergeIndex].start_time = now;
                    user.active_merges[activeMergeIndex].estimated_finish_time = 
                        new Date(now.getTime() + (waitTimeMinutes * 60 * 1000));
            ...
            if (user.merging_history) {
                ...
                if (existingMergeIndex !== -1) {
                    // Reset timer but keep progress
                    user.merging_history[existingMergeIndex].start_time = now;
                    user.merging_history[existingMergeIndex].estimated_finish_time = 
                        new Date(now.getTime() + (waitTimeMinutes * 60 * 1000));
                    user.merging_history[existingMergeIndex].last_update = now;
                    user.markModified('merging_history');
                }
            ...
            // IMPORTANT: Return success response, NOT an error with waiting time
            return res.status(200).json({
                success: true,
                message: `Evolve continued successfully with ${previousProgress}% progress. Timer started for next collection.`,
                data: {
                    current_progress: previousProgress,
                    anima_deducted: requiredAnima,
                    timing: {
        ...
        // IMPORTANT: DON'T SET PROGRESS TO 100% HERE
        // THIS SHOULD ONLY HAPPEN IN COLLECT ENDPOINT
        // The problem was that this code was setting progress to 100% on evolve
        // which was completing the merge instead of just starting a timer
        // user.creatures[creature1Index].upgrade_progress = 100;
        // user.creatures[creature2Index].upgrade_progress = 100;
        ...
        // Update the merging history record with completion info
        if (user.merging_history) {
            const existingMergeIndex = user.merging_history.findIndex(
                m => (m.creature1_id === creature1Id && m.creature2_id === creature2Id) ||
                     (m.creature1_id === creature2Id && m.creature2_id === creature1Id)
            ...
            // Create the merged creature here before using it anywhere
            ...
            if (existingMergeIndex !== -1) {
                ...
                const newMergeRecord = {
                    ...
                    creature2_name: creature2.name,
                    creature2_level: creature2.level,
                    start_time: new Date(now.getTime() - ((targetLevel === 11 ? 2 : targetLevel === 21 ? 5 : 10) * 60 * 1000)), // Approximate start time
                    completion_time: now,
                    ...
                    is_complete: true,
                    rarity: creatureTemplate.type,
                    wait_time_minutes: (targetLevel === 11 ? 2 : targetLevel === 21 ? 5 : 10), // Changed from 5/10/20 to 2/5/10
                    anima_spent: animaCost,
        ...
        // Note: The mergedCreature is already created above, don't recreate it here

        // Create the merged creature
        const mergedCreature = {
            _id: new mongoose.Types.ObjectId(), // Add proper MongoDB ID
            creature_id: creature1.creature_id,
            name: creatureTemplate.name,
            level: targetLevel,
            building_index: creature1.building_index,
            ...
            image: creatureTemplate.image,
            description: creatureTemplate.description,
            type: creatureTemplate.type || creature1.type || 'common', // Ensure type is preserved
            upgrade_progress: 0,
        ...
        // Define variables needed for response
        // Reuse the existing 'now' variable rather than redeclaring it
        let waitTimeMinutes;
        if (creatureTemplate.type === 'common') {
            waitTimeMinutes = targetLevel === 11 ? 2 : targetLevel === 21 ? 5 : 10;
        } else if (creatureTemplate.type === 'rare') {
        ...
        return res.status(200).json({
            ...
            message: isComplete ? 
                ...
                `Timer started. Wait ${waitTimeMinutes} minutes before collecting. No progress yet.`,
                anima_cost: animaCost,
                remaining_anima: user.currency.anima,
            timer: {
                wait_minutes: waitTimeMinutes,
